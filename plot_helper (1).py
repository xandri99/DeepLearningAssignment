...
def plot_history(model_history, model_name):
    ...
    ax.plot(model_history.history['categorical_accuracy'])
    ax.plot(model_history.history['val_categorical_accuracy'])
...
def show_random_images(data_directory, df_labels, file_column):
    ...
    for i in range(len(axes)):
        ...
        random_image = Image.open(data_directory + row["image ID"].values[0])
        axes[i].set_title(row["class label"].values[0], fontsize=14) 
